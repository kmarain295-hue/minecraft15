 ...
 *   - STRAFE (hold): BUZZ the pet drone streams its chin gun at the ground
 *     point ahead of you (same as holding the keyboard G key).
 *   - MISSILE: BUZZ drops one wing-pod missile on that spot (keyboard H).
...
export interface TouchGameApi {
  ...
  exitToLobby(): void;
  /** Enter/exit the jeep when close (same as the keyboard E key). */
  enterVehicle(): void;
  /** BUZZ STRAFE pressed (streams the drone's chin gun while held, like G). */
  droneGunDown(): void;
  /** BUZZ STRAFE released. */
  droneGunUp(): void;
  /** BUZZ drops one missile on the point you face (same as the H key). */
  droneMissile(): void;
}
...
export default function TouchControls({ apiRef }: TouchControlsProps) {
  ...
  const vehicleDown = (event: React.PointerEvent<HTMLButtonElement>) => {
    pressCapture(event);
    apiRef.current?.enterVehicle();
  };

  const droneStrafeDown = (event: React.PointerEvent<HTMLButtonElement>) => {
    pressCapture(event);
    apiRef.current?.droneGunDown();
  };
  ...
  const droneMissileDown = (event: React.PointerEvent<HTMLButtonElement>) => {
    pressCapture(event);
    apiRef.current?.droneMissile();
  };
  ...
  return (
    ...
    >
      ...
      {/* jeep: enter/exit when close (mirrors the keyboard E key) */}
      <RoundButton
        icon={Car}
        label="Car"
        size={56}
        ...
        style={{
          left: '1.6rem',
          bottom: `calc(16.4rem + ${SAFE_B})`,
        }}
      />

      {/* BUZZ airstrike: hold STRAFE (drone gun), tap MISSILE (drone bomb) */}
      <RoundButton
        icon={Target}
        label="Strafe"
        size={56}
        iconClass="h-6 w-6"
        onDown={droneStrafeDown}
        onUp={droneStrafeUp}
        onLostCapture={droneStrafeUp}
        style={{
          left: '1.6rem',
          bottom: `calc(21.8rem + ${SAFE_B})`,
        }}
      />
      <RoundButton
        icon={Rocket}
        ...
        iconClass="h-6 w-6"
        danger
        onDown={droneMissileDown}
        style={{
          left: '1.6rem',
          bottom: `calc(27.2rem + ${SAFE_B})`,
        }}
      />
