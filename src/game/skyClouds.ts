...
export interface CloudFrameInfo {
  ...
  winter?: number;
  /** 0..1 — player's hot-biome factor (dune seas + badlands): the deck
   *  takes a warm dusty bake. */
  hot?: number;
  /** 0..1 — player's volcano factor: the deck darkens to smoky ash. */
  ash?: number;
}
...
const HOT_LIT = new THREE.Color(0xe8c9a0); // warm dusty bake over hot biomes
const HOT_SHADOW = new THREE.Color(0xc9a075);
const ASH_LIT = new THREE.Color(0x4a3a30); // smoky ash over the volcano
const ASH_SHADOW = new THREE.Color(0x2e241d);
...
export function createSkyClouds(
  ...
  return {
    update(info: CloudFrameInfo) {
      ...
      // hot biomes bake the deck dusty-warm; the volcano smokes it dark
      const hot = THREE.MathUtils.clamp(info.hot ?? 0, 0, 1);
      if (hot > 0.001) {
        lit.lerp(HOT_LIT, hot * 0.5);
        shadow.lerp(HOT_SHADOW, hot * 0.5);
      }
      const ash = THREE.MathUtils.clamp(info.ash ?? 0, 0, 1);
      if (ash > 0.001) {
        lit.lerp(ASH_LIT, ash * 0.7);
        shadow.lerp(ASH_SHADOW, ash * 0.7);
      }
      const silver =
