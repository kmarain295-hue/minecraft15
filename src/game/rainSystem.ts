...
const FAR_DROPS = 170000; // budget scaled up with the taller sky column so
                         // on-screen density stays identical to the old height
const WORLD_HALF = 6400; // terrain spans 12,800 x 12,800 world units
const FAR_TOP = 4400; // well above the raised cloud deck (2000) — wraps stay
                     // in open sky and drops still dive through the clouds
...
const NEAR_TOP = 480; // spawn height above the player's feet
...
export interface RainHandle {
  ...
  /** Debug: whether the global mute button currently silences the bed. */
  audioMuted(): boolean;
  /** Debug: the bed's live gain (null until the private context exists). */
...
export function createRain(
  ...
  // The HUD sound button (via the global mute bus) must silence this bed
  // too — it runs on its own AudioContext, OUTSIDE audio.ts's master gain.
  // Mute flips fade the bed out right away; unmute needs nothing here (the
  // per-frame update below re-raises the gain to the storm intensity).
  let muted = false;
