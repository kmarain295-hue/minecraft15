 ...
 *  above the raised cumulus deck (2000) but under the cirrus veil. */
const SKY_HEIGHT = 3400;
...
/**
 * Plumage recolour for the vertex-colour "texture" shipped in the GLB's
 * COLOR_0 attribute (the model has no image texture — its texture IS the
 * vertex colours). Rotates every vertex colour around the hue circle by
 * `hueShift` with a slight saturation lift (`satBoost`), leaving white /
 * black markings untouched. Input/output are linear-space floats.
 *  - shift 0.62: red body -> royal blue, blue wings -> gold  (macaw)
 *  - shift 0.32: red body -> acid green, blue wings -> magenta (toxin)
 */
function shiftPlumageHue(
  r: number,
  g: number,
  b: number,
  hueShift: number,
  ...
  const d = max - min;
  if (d < 1e-5) return [r, g, b]; // pure gray/black — no hue to rotate
  const l = (max + min) / 2;
  ...
  return [t2c(h + 1 / 3), t2c(h), t2c(h - 1 / 3)];
}
...
export function createBirdFlock(
  ...
  new GLTFLoader().load(BIRD_MODEL_URL, function (gltf) {
    ...
    // Three plumages split across the VISIBLE flock (only the first
    // DRAW_COUNT birds are ever rendered — drawRange below):
    //   8 of 26 (30.8%) classic red   — untouched original colours
    //   8 of 26 (30.8%) blue-and-gold — hue +0.62 macaw recolour
    //  10 of 26 (38.5%) toxic green   — hue +0.32 acid recolour
    // Bird -> variant is a fixed pseudo-random mapping: (bird * 17) mod 26
    // is a permutation (gcd(17, 26) = 1), so the three stride bands scatter
    // every plumage evenly across the sky.
    const posSrc = birdGeo.getAttribute('position').array as ArrayLike<number>;
    const colSrc = birdGeo.getAttribute('color').array as ArrayLike<number>;
    const floatsPerBird = birdVertexCount * 3;
    const TOXIN_COUNT = 10; // largest band: the "remaining 40%"
    const MACAW_COUNT = 8; // sits between toxin and red in the permutation
    const macawColors = new Float32Array(colSrc.length);
    const toxinColors = new Float32Array(colSrc.length);
    for (let v = 0; v < colSrc.length; v += 3) {
      const [mr, mg, mb] = shiftPlumageHue(
        colSrc[v],
        colSrc[v + 1],
        colSrc[v + 2],
        0.62,
      ...
      macawColors[v + 1] = mg;
      macawColors[v + 2] = mb;
      const [tr, tg, tb] = shiftPlumageHue(
        colSrc[v],
        colSrc[v + 1],
        colSrc[v + 2],
        0.32,
    ...
    for (let i = 0; i < totalVertices; i++) {
      const bIndex = i % floatsPerBird;
      const bird = Math.floor(i / floatsPerBird);
      let src: ArrayLike<number> = colSrc; // classic red original
      if (bird < DRAW_COUNT) {
        const band = (bird * 17) % DRAW_COUNT;
        if (band < TOXIN_COUNT) src = toxinColors;
        else if (band < TOXIN_COUNT + MACAW_COUNT) src = macawColors;
      }
      vertices.push(posSrc[bIndex]);
      color.push(src[bIndex]);
    }
